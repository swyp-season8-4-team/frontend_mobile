...
final Provider<LeaveMateMemberUsecase> leaveMateMemberUsecaseProvider =
    Provider<LeaveMateMemberUsecase>(
      (Ref ref) => LeaveMateMemberUsecase(
        repository: ref.read(mateMemberRepositoryProvider),
      ),
    );

class LeaveMateMemberUsecase
    extends Usecase<MateMemberModel, LeaveMateMemberParams> {
  LeaveMateMemberUsecase({required this.repository});
  ...
  Future<Result<MateMemberModel, CustomException>> call({
    ...
    return await repository.leaveMateMember(params: params);
  }
}
